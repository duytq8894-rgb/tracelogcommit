// ============================================================================
// MẢNH GHÉP (snippet) cho top environment — KHÔNG phải testbench đầy đủ.
//
// Dán đoạn này vào BÊN TRONG module top/testbench đã có sẵn:
//   - instance `i_dut` = ara_system  (lõi CVA6 + Ara)
//   - `clk`, `rst_n`                 (đồng hồ + reset của môi trường)
//   - import instr_tracer_synth_pkg::*;  import ara_pkg::*;
//
// Nó gọi 3 instance để bật tracer tổng hợp trong môi trường và cho chạy:
//   (1)  i_tracer      = instr_tracer_synth       -> log lệnh SCALAR
//   (1b) i_vec_tracer  = instr_tracer_synth_vec   -> log lệnh VECTOR (RVV)
//   (2)  i_sink        = instr_tracer_synth_sink  -> in trace_hart_0_commit.synth.log
//
// Tap toàn bộ qua tham chiếu phân cấp (i_dut.i_ariane.* / i_dut.i_ara.*), không
// sửa DUT. Chỉnh NrLanes cho khớp cấu hình Ara.  (Ví dụ top hoàn chỉnh: xem
// hardware/tb/tb_ara_system_trace.sv.)
// ============================================================================
...
//     - vtype/vl/fflags : i_dut.i_ara.i_dispatcher.*        (vsew/vlmul/vl + fflags)
//     - ghi VRF        : i_dut.i_ara.gen_lanes[L].i_lane.*  (shadow VRF)
