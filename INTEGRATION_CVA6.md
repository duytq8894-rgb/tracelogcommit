...
> Các tín hiệu LSU trên dùng cho token `mem 0x<addr> 0x<data>` (load/store). Nếu
> không cần in địa chỉ bộ nhớ, có thể nối `st_valid_i=ld_valid_i=0`,
> `flush_addr_i=0` và bỏ qua phần còn lại.
